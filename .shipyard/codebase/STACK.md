...
| `http_req` | 0.11 | Lightweight HTTP client (used for update checks) |

Evidence: `Cargo.toml` lines 108, 159, 183-184; `wezterm-gui/src/update.rs` lines 4-5
...
## Findings: WASM-Related Dependencies

No WASM runtime (wasmtime, wasmer, etc.) is present in the workspace dependencies. The only WASM reference is inside the vendored HarfBuzz C library's experimental Rust WASM binding:

- `deps/harfbuzz/harfbuzz/src/wasm/rust/harfbuzz-wasm/src/lib.rs` — this is upstream HarfBuzz source, not an ArcTerm addition.

The planned `arcterm-wasm-plugin` crate (referenced in `CLAUDE.md`) does **not exist yet** as a workspace member.
- Evidence: `Cargo.toml` `[workspace] members` list contains no `arcterm-*` entries.
...
| WASM runtime | None present | Observed |
| AI crates | None present | Observed |
