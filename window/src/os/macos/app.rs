...
const CLS_NAME: &str = "ArcTermAppDelegate";
    ...
    unsafe {
        match config::configuration().window_close_confirmation {
            ...
            WindowCloseConfirmation::AlwaysPrompt => {
                ...
                let message_text = nsstring("Quit ArcTerm?");
                let info_text = nsstring("Detach and close all panes and quit ArcTerm?");
...
extern "C" fn application_dock_menu(
    ...
    let new_window_item =
        MenuItem::new_with("New Window", Some(sel!(arctermPerformKeyAssignment:)), "");
    new_window_item
    ...
    Class::get(CLS_NAME).unwrap_or_else(|| {
        ...
        unsafe {
            ...
            cls.add_method(
                ...
                arcterm_perform_key_assignment as extern "C" fn(&mut Object, Sel, *mut Object),
            );
            cls.add_method(
